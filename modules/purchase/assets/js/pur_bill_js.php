<script>
  ...
  function pur_calculate_total(from_discount_money) {
    ...
    $.each(rows, function() {
      ...
      billed_qty = $(this).find('[data-billed-quantity]').val();
      if (billed_qty === '') {
        billed_qty = 0;
        $(this).find('[data-billed-quantity]').val(0);
      }
      if (billed_qty > quantity) {
        alert_float('warning', 'Billed Quantity cannot be greater than Ordered Quantity');
      ...
      _amount = accounting.toFixed($(this).find('td.rate input').val() * billed_qty, app.options.decimal_places);

      $(this).find('.remaining_quantity').html(quantity - billed_qty);
