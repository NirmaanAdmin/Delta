<script>
  function uploadfilecsv(event) {
    ...
    if (($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')) {
      ...
      $.ajax({
        url: admin_url + 'meeting_management/minutesController/import_file_xlsx_critical_tracker_items',
        method: 'post',
        data: formData,
        contentType: false,
        processData: false
        ...
        $('.invoice-item table.critical-tracker-items-table.items tbody').append(response.list_item);
