<script>
  function uploadfilecsv(event) {
    ...
    if (($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')) {
      ...
      $.ajax({
        url: admin_url + 'meeting_management/agendaController/import_file_xlsx_mom_items',
        method: 'post',
        data: formData,
        contentType: false,
        processData: false
        ...
        $('.mom-items table.mom-items-table.items tbody').html('');
        $('.mom-items table.mom-items-table.items tbody').append(response.list_item);
       
        init_selectpicker();
        pur_clear_item_preview_values('.mom-items');
