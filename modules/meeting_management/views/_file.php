            ...
            <div class="row">
               <div class="col-md-12 border-right project_file_area">
                  <?php if (!empty($file->external) && $file->external == 'dropbox') { ?>
                     <a href="<?php echo pur_html_entity_decode($file->external_link); ?>" target="_blank" class="btn btn-info mbot20">
                        <i class="fa fa-dropbox" aria-hidden="true"></i> <?php echo _l('open_in_dropbox'); ?>
                     </a><br />
                  <?php } ?>

                  <?php
                     $path = get_upload_path_by_type('meeting_management') . 'agenda_meeting/' . $file->rel_id . '/' . $file->file_name;
                     $file_url = base_url('uploads/meetings/agenda_meeting/' . $file->rel_id . '/' . $file->file_name);
                        
                     if (is_image($path)) { ?>
                        <img src="<?php echo $file_url; ?>" class="img img-responsive img_style">
                  ...
                  <?php } else if (strpos($file->file_name, '.pdf') !== false && empty($file->external)) { ?>
                        <iframe src="<?php echo $file_url; ?>" height="100%" width="100%" frameborder="0"></iframe>
                  
                  <?php } else if (
                     (strpos($file->file_name, '.xls') !== false || strpos($file->file_name, '.xlsx') !== false || strpos($file->file_name, '.doc') !== false || strpos($file->file_name, '.docx') !== false) && empty($file->external)
                  ) { ?>
                        <iframe src='https://view.officeapps.live.com/op/embed.aspx?src=<?php echo urlencode($file_url); ?>' width='100%' height='100%' frameborder='0'></iframe>
...
<?php require 'modules/meeting_management/assets/js/_file_js.php'; ?>
