<script>
	(function() {
		...
		initDataTable('.table-assets_management', admin_url + 'fixed_equipment/all_asset_table', [0, 29], [0, 29], fnServerParams, [3, 'desc']);
	 ...
	 * deilvery status 
	 */
	function deilvery_status(el, id) {
		"use strict";
		var asset_name = $(el).data('asset_name');

		// var model = $(el).data('model');
		$('#deilvery_status').modal('show');
		$('#deilvery_status input[name="item_id"]').val(id);
		$('#deilvery_status .modal-header .add-title').text(asset_name);
		// $('#check_in input[name="asset_name"]').val(asset_name);
		// $('#check_in input[name="model"]').val(model);
