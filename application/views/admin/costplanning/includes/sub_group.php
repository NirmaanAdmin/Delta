 ...
 <thead>
  ...
  <tbody>
    ...
    <?php 
     ...
     ?>
    <tr>
        <td><?php echo pur_html_entity_decode($sub_group_type['id']); ?></td>
        <td><?php echo pur_html_entity_decode($sub_group_type['sub_group_code']); ?></td>
        <td><?php echo pur_html_entity_decode($sub_group_type['sub_group_name']); ?></td>
        <td><?php echo pur_html_entity_decode($group_name); ?></td>
        <td><?php echo pur_html_entity_decode($sub_group_type['order']); ?></td>
        <td><?php if($sub_group_type['display'] == 0){ echo _l('not_display'); }else{echo _l('display');} ?></td>
        <td><?php echo pur_html_entity_decode($sub_group_type['note']); ?></td>

        <td>
          <a href="#" onclick="edit_sub_group_type(this,<?php echo pur_html_entity_decode($sub_group_type['id']); ?>); return false;" data-sub_group_code="<?php echo pur_html_entity_decode($sub_group_type['sub_group_code']); ?>" data-name="<?php echo pur_html_entity_decode($sub_group_type['sub_group_name']); ?>" data-group_id="<?php echo pur_html_entity_decode($sub_group_type['group_id']); ?>" data-order="<?php echo pur_html_entity_decode($sub_group_type['order']); ?>" data-display="<?php echo pur_html_entity_decode($sub_group_type['display']); ?>" data-note="<?php echo pur_html_entity_decode($sub_group_type['note']); ?>" class="btn btn-default btn-icon"><i class="fa fa-pencil-square"></i>
           </a>

            <a href="<?php echo admin_url('costplanning/delete_sub_group/'.$sub_group_type['id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
        </td>
    </tr>
    <?php } ?>
 </tbody>
...
<div class="modal fade" id="sub_group_type" tabindex="-1" role="dialog">
        <div class="modal-dialog setting-handsome-table">
          <?php echo form_open_multipart(admin_url('costplanning/sub_group'), array('id'=>'add_sub_group')); ?>
